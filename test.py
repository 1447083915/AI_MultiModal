...
class MultiModalModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_size, num_classes, use_image=True, use_text=False):
...
def draw_plot(accuracy_list, loss_list, val_loss_list, cost, epochs):
    ...
    # 绘制准确度、训练损失和验证损失曲线
    ax.plot(epochs_list, accuracy_list, label='Training Accuracy', color='blue', marker='o')
    ax.plot(epochs_list, loss_list, label='Training Loss', color='red', marker='o')
    # ax.plot(epochs_list, val_loss_list, label='Validation Loss', color='green', marker='o')
...
# 创建数据集实例
dataset = CustomDataset(data_folder, train_txt_file, dict_file, transform=transform)
# print(dataset[10])
...
for epoch in range(num_epochs):
    ...
    end_of_this_epoch = time.time()

    print(
    f'Epoch [{epoch + 1}/{num_epochs}], Loss: {loss.item():.4f}, Val Loss: {val_loss.item():.4f}, '
    f'Accuracy: {accuracy:.4f}, cost time:{(end_of_this_epoch-start):.4f}')

# 绘制图像
draw_plot(accuracy_list, loss_list, val_loss_list, train_cost, num_epochs)
